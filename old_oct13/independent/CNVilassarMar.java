 ...
 * @author Victor
 */
public class CNVilassarMar implements StationFetcher { // TODO es molt semblant a CNHospitaletVandellos, nomes canvien dos numeros

    @Override
    public Dada fetch() {
        Dada ret = new Dada();
        
        String raw = Globals.getHTML("http://www.cnvilassar.com/ftp/download/downld02.txt");
        ...
        try {
            ret.barometer = Float.parseFloat(arr[16]);
        }catch (NumberFormatException ex){}
        try {
            ret.rain = Float.parseFloat(arr[17]);
        }catch (NumberFormatException ex){}
